	...
	def Step(self, settings):
		...
		for word_k in range(0, self.nGrams.nWords):
			...
			if phase == 1:
				...
				oldSize[1] = (self.maxSize - self.minSize) * (
							self.nGrams.counts[word_k][date_k - len(self.nGrams.counts[word_k])] / float(self.nGrams.maxCount)) + self.minSize
		...
		if self.saveFrames:
			pygame.image.save(self.screen,
							  self.saveFolder  + int(self.frameN).__format__('').zfill(10)
							  + '.png')
